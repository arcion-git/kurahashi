                  ...
                  <tr class="repeatorder_item" id="{{$repeatcart->id}}">
                    ...
                    <td colspan="8" class="order-table repeat_order_table">
                          ...
                          <td class="text-center head-price">
                            <input name="repeatorder[{{$val->id}}][price]" class="price text-center form-control" value="@if ( Auth::guard('admin')->check() ){{$val->price}}@else{{number_format($val->price)}}@endif" pattern="^[0-9]+$" title="0から9の半角数字" required>
                          </td>
